...
The above fallback scenario is pretty common. In fact, it's often the case that you'll cache some set of resources and want to simply say _"if I've got it, use it, if not, go to the network for it"_. Or even _"if I've got it, use it, if not, try to go to the network and if *that* doesn't work, use this one instead"_. Long winded, but you get where this is going.

In fact, it seems a darned shame to have to go through `onfetch` for all of this stuff when you can just say up-front what you want to happen later on.

Enter static routes.

The way to think of static routes are as "things to try first". Every fetch that would otherwise generate an `onfetch` is first mapped against a list of static routes that an app can set up at any time. Once installed, fetches for route-managed URLs will only be sent to the controller in cases where no default is provided or for which the default somehow ends in failure.

Here's an example that sets up static routes for some of the resources we've seen before:
...
this.addEventListener("install", function(e) {
  ...
  // Set up routes via the global "staticRoutes" property
  this.staticRoutes.addCache(shellResources);
...
This controller _could_ have an `onfetch` handler, perhaps to deal with non-shell resource requests or situations that arise if the cache isn't correctly populated, but this controller won't be installed or become active until the cache is ready, so there's not much need.

What we might what to do, however, is add individual mappings or handle something other than exact URL match (which is the behavior of adding a cache to the static routes).

Here's how to add a mapping to many URLs based on a star match, tries to use the network first, and tries a cache item second:
...
this.addEventListener("install", function(e) {
  ...
  var base = "http://videos.example.com";
  var rickroll = new URL("/assets/v1/nevergonnagiveyouup.webm", base);

  var videos = new Cache(rickroll);
  this.caches.set("videos-v1", videos);
  this.staticRoutes.add("/videos/*", ["network", videos.match(rickroll)]);
  e.waitUntil(videos.ready());
...
this.addEventListener("install", function(e) {
  ...
  this.staticRoutes.add("/videos/stale_content.webm",
    [ new NetworkSource("/videos/fresh_content.webm"),
      new CacheSource("videos-v1", rickroll) ]
  );

  this.staticRoutes.add("/videos/*",
    [ "network",
      new CacheSource("videos-v1", rickroll) ]
  );
...
Any time you're tempted to put a cache item directly in a static mapping, _stop_, think twice, and then use a `CacheSource` instead. It's what `addCache()` does under the covers for all of the URLs in the cache, and it's what your apps should do when manually mapping resources.
