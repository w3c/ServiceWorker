...
The `caches.match()` dance is a bit wordy, so to cut this short there's a `match` convenience method on the global `caches` object to make our `onfetch` handler shorter but instead of taking one parameter (the URL), it takes two (the cache name and the URL):
...
this.addEventListener("fetch", function(e) {
  ...
  if (url == inventoryURL) {
    e.respondWith(
      fetch(url).then(
        null,
        function() { return caches.match("shell-v1", fallbackInventory); }
      )
    );
  }
});
